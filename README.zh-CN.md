...
本地 Chrome Manifest V3 扩展：整理当前打开的标签页，按域名/自定义规则聚类，显示 favicon、标题、页面概览、重复标记，并提供关闭单个、关闭整组、关闭重复、勾选批量关闭、保存稍后再看、Archive 和关闭历史查看。
...
安装时 Chrome 会提示扩展需要访问历史记录、标签页、最近关闭会话和网页内容。这些权限用于：

- `tabs`：读取当前打开的标签，并执行切换/关闭操作。
- `history`：只做本地熟悉度统计，不展示已关闭历史页面。
- `sessions`：读取 Chrome 最近关闭的标签/窗口，并在用户点击后恢复。
- `scripting`：读取当前打开页面的标题、描述、正文片段，用于生成页面概览。
- `storage`：保存主题、选择状态、Saved for later、Archive 和本地面板关闭历史。
...
- Recently closed 组合展示 Chrome 最近关闭会话和从本面板关闭的标签。
- Recently closed 支持原位恢复 Chrome session 项，也支持用安全 URL 重新打开面板记录项。
...
- `sessions` 只用于展示和恢复 Chrome 最近关闭的标签/窗口，数量受 Chrome 最近关闭列表限制。
- 面板关闭历史只保存安全本地字段，最多保留 30 天或 200 条。
